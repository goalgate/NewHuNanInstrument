...
public class UpdateService extends Service {
    ...
    private void CopySourceFile() {
        if (AppUtils.getAppVersionName().equals("2.0") && config.getBoolean("CopySourceFileVer20", true)) {
//            if (!new File(UpdateConstant.ORIGINAL_APK_PATH).exists()) {
            Observable.create((emitter) -> {
                emitter.onNext(ApkUtils.copyfile(
                        new File(ApkUtils.getSourceApkPath(UpdateService.this, UpdateConstant.TEST_PACKAGENAME)),
                    ...
                    .subscribe((l) -> {
                        ...
                        if (status) {
                            ...
                            config.put("CopySourceFileVer20", false);
                            autoUpdate();
                        } else {
...
//            }
    ...
    private void autoUpdate() {
        ...
        if (config.getBoolean("CopySourceFileVer20", true)) {
...
//        String pattern = "yyyy-MM-dd '16:00:00'";
        ...
        try {
            ...
            TimerTask task = new TimerTask() {
                ...
                public void run() {
                    // 要执行的代码
                    CJYHelper.getInstance(UpdateService.this).reboot();
                    Log.e("信息提示", "关机了");
                }
            };
