...
public class NewNMGUpdateService extends Service {
    ...
    private void CopySourceFile() {
        if (AppUtils.getAppVersionName().equals("2.1") && config.getBoolean("CopyFileVer_21", true)) {
//            if (!new File(UpdateConstant.ORIGINAL_APK_PATH).exists()) {
            Observable.create((emitter) -> {
                emitter.onNext(ApkUtils.copyfile(
                        new File(ApkUtils.getSourceApkPath(NewNMGUpdateService.this, UpdateConstant.TEST_PACKAGENAME)),
                    ...
                    .subscribe((l) -> {
                        ...
                        if (status) {
                            ...
                            config.put("CopyFileVer_21", false);
                            autoUpdate();
                        } else {
...
//            }
    ...
    private void autoUpdate() {
        ...
        if (config.getBoolean("CopyFileVer_21", true)) {
...
//        String pattern = "yyyy-MM-dd '10:00:00'";
        ...
        try {
            ...
            TimerTask task = new TimerTask() {
                ...
                public void run() {
                    // 要执行的代码
                    CJYHelper.getInstance(NewNMGUpdateService.this).reboot();
                    Log.e("信息提示", "关机了");
                }
            };
