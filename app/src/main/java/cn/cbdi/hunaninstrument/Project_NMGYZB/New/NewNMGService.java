...
public class NewNMGService extends Service implements IOutputControlView {

    private String TAG = NewNMGService.class.getSimpleName();
...
//                .subscribe((l) -> checkOnline());
    ...
    private void getPic() {
        ...
        if (employers.size() > 0) {
            for (Employer employer : employers) {
                RetrofitGenerator.getNewNMGApi().queryPersonInfo("recentPic", config.getString("key"), employer.getCardID())
    ...
    private void reUpload() {
        ...
        for (final ReUploadBean bean : list) {
            RetrofitGenerator.getNewNMGApi().withDataRr(bean.getMethod(), config.getString("key"), new String(bean.getContent()))
                    .subscribeOn(Schedulers.single())
                    .unsubscribeOn(Schedulers.single())
                    .observeOn(Schedulers.single())
                    .subscribe(new Observer<ResponseBody>() {
    ...
    private void syncData() {
        ...
        RetrofitGenerator.getNewNMGApi().withDataRr("updatePerson", config.getString("key"), obj.toString())
    ...
    private void testNet() {
        RetrofitGenerator.getNewNMGApi().noData("testNet", config.getString("key"))
                .subscribeOn(Schedulers.io())
                .unsubscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<ResponseBody>() {
    ...
    private void alarmRecord() {
        ...
        RetrofitGenerator.getNewNMGApi().withDataRr("alarmRecord", config.getString("key"), alarmRecordJson.toString())
                .subscribeOn(Schedulers.io())
                .unsubscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<ResponseBody>() {
