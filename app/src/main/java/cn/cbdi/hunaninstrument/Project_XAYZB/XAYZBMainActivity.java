...
public class XAYZBMainActivity extends BaseActivity implements NormalWindow.OptionTypeListener {
    ...
    private void UIReady() {
        ...
        alert_password.PasswordViewInit(() -> {
            normalWindow = new NormalWindow(XAYZBMainActivity.this);
            normalWindow.setOptionTypeListener(XAYZBMainActivity.this);
    ...
    void openService() {
        intent = new Intent(XAYZBMainActivity.this, AppInit.getInstrumentConfig().getServiceName());
        startService(intent);
    }
                ...
                if (employer.getType() == 1) {
                    if (DoorOpenOperation.getInstance().getmDoorOpenOperation().equals(DoorOpenOperation.DoorOpenState.Locking)) {
                        ...
                        Observable.timer(60, TimeUnit.SECONDS).subscribeOn(Schedulers.newThread())
                                .compose(XAYZBMainActivity.this.<Long>bindUntilEvent(ActivityEvent.PAUSE))
                                .observeOn(AndroidSchedulers.mainThread())
...
//                }
    ...
    public void onGetFaceIdentityEvent(FaceIdentityEvent event) {
        if (isForeground(AppInit.getContext(), XAYZBMainActivity.class.getName())) {
            fp.FaceIdentify_model();
        }
