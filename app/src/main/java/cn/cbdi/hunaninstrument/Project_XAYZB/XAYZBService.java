package cn.cbdi.hunaninstrument.Project_XAYZB;
...
public class XAYZBService extends Service implements IOutputControlView {

    private String TAG = XAYZBService.class.getSimpleName();
    ...
    private void reUpload() {
        ...
        for (final ReUploadBean bean : list1) {
            RetrofitGenerator.getXAYZBApi().faceUpload("faceUpload", paramsMap.get("daid"), paramsMap.get("pass"), bean.getContent())
    ...
    private void syncData() {
        ...
        RetrofitGenerator.getXAYZBApi().GeneralPersionInfo(map)
...
//                Keeper keeper = new Keeper("441302199308100538".toUpperCase(),
...
//                Keeper keeper = new Keeper("44128219830820403X".toUpperCase(),
//                        "彭艺煊", FileUtils.bitmapToBase64(songbitmap), null, null,
//                        user.getUserId(), user.getFeature());
//                mdaoSession.getKeeperDao().insertOrReplace(keeper);
//                Log.e("myface", "彭艺煊" + "人脸特征已存");
//
//            }
//        }
        ...
        if (employers.size() > 0) {
            for (Employer employer : employers) {
                RetrofitGenerator.getXAYZBApi().recentPicNew("recentPic", paramsMap.get("daid"), paramsMap.get("pass"), employer.getCardID())
                        .subscribeOn(Schedulers.single())
                        .unsubscribeOn(Schedulers.single())
                        .observeOn(Schedulers.single())
    ...
    private void testNet() {
        ...
        RetrofitGenerator.getXAYZBApi().GeneralUpdata(map)
                .subscribeOn(Schedulers.io())
                .unsubscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<String>() {
